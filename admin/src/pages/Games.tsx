...
const Games = () => {
  ...
  const openEditModal = (game: any) => {
    ...
    setEditIsFeatured(!!game.isFeatured);
    setEditIsNew(!!game.isNew);
  };
    ...
    <div className="games-page">
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          {view !== 'managed' && (
            <button onClick={() => setView('managed')} className="btn-outline">
              Back to Managed
            </button>
          )}
          {view !== 'import' && (
            ...
            <tbody>
              {(view === 'managed' ? managedGames : externalGames).map((game, index) => (
                <tr key={game.id || index}>
                  <td><img src={game.thumbnail} alt="" className="thumbnail-sm" /></td>
                  <td style={{ fontWeight: '700' }}>{game.title}</td>
                  <td><span className="badge badge-lime">{game.category}</span></td>
                  <td>
                  ...
                  <td>
                    {view === 'managed' ? (
                      <div style={{ display: 'flex', gap: '10px' }}>
                        <button onClick={() => openEditModal(game)} className="btn-outline" style={{ padding: '6px 12px', fontSize: '11px' }}>Edit</button>
                        <button onClick={() => handleDelete(game)} className="btn-outline" style={{ padding: '6px 12px', fontSize: '11px', color: 'var(--error)', borderColor: 'rgba(255, 75, 75, 0.2)' }}>Delete</button>
                        <button onClick={() => handleQuickUpdate(game)} className="btn-primary" style={{ padding: '6px 12px', fontSize: '11px' }}>Update</button>
                      </div>
                    ) : (
                      <button onClick={() => handleSave(game)} disabled={saving === game.title} className="btn-primary" style={{ padding: '8px 16px', fontSize: '12px' }}>
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <div className="modal-buttons">
              ...
              <button onClick={() => setEditingGame(null)} className="btn-outline">Cancel</button>
            </div>
          </div>
        </div>
      )}
